...
每颗生成的星球都会产生一个 **星球代码**（显示在生成按钮下方），其中编码了随机种子、所有影响生成的滑块、平面地图投影、中心经纬度、地图倾角 γ、地图缩放，以及仍适用于当前板块布局的板块编辑；视图模式、相机位置、图层显示和线框等纯可视化选项不会写入代码。未编辑星球为 29 个字符；通过重建应用过的板块编辑会扩展代码，把切换过的板块包含进去。旧版本的 13-27 字符代码仍受支持，缺失的新参数会使用当前默认值。拖动任意会影响生成或地图的参数时，代码框会实时刷新。分享星球的方法：
...
js/
  ...
  planet-code.js        星球代码编解码（种子 + 生成/地图参数 → base36）
