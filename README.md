...
- **高度图导入**：可将自己的等距柱状黑白高度图（地球、火星、手绘地图）导入到 3D 星球。黑色像素表示海洋，越亮表示陆地越高。导入页（`import.html`）会在导入地形上运行完整气候模拟（风、降水、温度、柯本），并可选择地形雕刻（平滑、侵蚀、山脊锐化）。支持 PNG、JPEG、WebP。
...
然后在浏览器打开 **http://localhost:8000**。无需安装依赖，也无需构建。普通打开页面时会先显示一张已生成世界的静态预览背景，不会立刻启动新世界生成；打开带有效星球代码的 URL 时仍会自动加载该世界。
...
- **导入**（`import.html`）— 导入自己的等距柱状黑白高度图，在 3D 球体上查看并运行气候模拟。黑色（0）= 海洋，更亮 = 更高海拔。支持 PNG、JPEG 和 WebP。

### 分享星球

每颗生成的星球都会产生一个 **星球代码**（显示在生成按钮下方），其中编码了随机种子、所有真正影响世界生成的滑块（包括细节、板块/大陆设置、地形雕刻、气候偏移、大陆占比等），以及仍适用于当前板块布局的板块编辑；视图模式、相机位置、图层显示、线框、平面地图投影、投影视点、地图倾角 γ 和地图缩放等纯可视化选项不会写入代码。未编辑星球为 20 个字符；通过重建应用过的板块编辑会扩展代码，把切换过的板块包含进去。旧版本的 13-29 字符代码仍受支持，缺失的新参数会使用当前默认值；旧代码中曾包含的地图视角字段会被读取但不会应用到当前视图。拖动任意会影响生成的参数时，代码框会实时刷新。分享星球的方法：
...
| 控制项 | 范围 | 默认值 | 说明 |
|---|---|---|---|
| 塑造种子 | 任意文本 | 空 | 输入或随机生成一个文本种子并应用，只会改变本节里除“细节”以外的塑造参数，不会触碰地形雕刻、气候、地图或显示选项 |
...
- **地图投影** 下拉框（仅平面地图模式）：切换 d3-geo 核心投影（等距圆柱、墨卡托、横轴墨卡托、兰伯特等面积方位投影、等距方位、心射、正射、球极平射、阿尔伯斯等面积圆锥投影、兰伯特正形圆锥投影、等面积圆锥、等距圆锥、自然地球、Equal Earth）以及 d3-geo-projection 扩展库投影。
...
- **导出世界包 ZIP**：默认打包地形图、卫星图、气候图和全范围高度图；陆地遮罩和其他检视图层可在弹窗中勾选加入。可按住一个可选图层复选框并拖过列表，批量选中或取消经过的选项。如果尚未计算气候，会在导出前自动运行。
...
控制面板可通过侧边栏标题中的 **«** 按钮折叠和展开。小屏幕（≤ 768px）上，侧边栏会变成带拖拽把手的底部面板，初始为折叠状态，只显示把手和标题。向上拖动或点击把手即可展开。普通加载时先显示静态预览背景；每次生成期间都会显示带加载指示器、标题和进度条的全屏遮罩，后续构建时会让上一颗星球在背后变暗。阶段标签（塑造、板块、海洋、山脉、绘制）会随流程推进而更新。
...
项目是纯静态站点，不需要 Node.js 构建步骤。最简单的发布方式是在仓库的 **Settings → Pages** 中选择 **Deploy from a branch**，并把 Source 指向 `gh-pages` 分支的 `/` 根目录。

原始仓库保留 `CNAME` 用于 `orogen.studio` 自定义域名；Fork 仓库发布到 GitHub 项目页时，请在发布分支中移除 `CNAME`，最终地址会是 `https://<user>.github.io/planet_heightmap_generation/`。页面导航使用相对路径，可同时兼容自定义域名和 GitHub 项目页路径。仓库内也保留了 release 触发的 Pages workflow，适合有 GitHub Actions Pages 权限的原仓库继续使用。

## 项目结构

```text
.github/workflows/
  deploy.yml            release 触发的 GitHub Pages workflow（原仓库）
...
js/
  ...
  planet-code.js        星球代码编解码（种子 + 生成参数 → base36）
  ...
  plate-physics.js      板块边界类型、运动与密度辅助计算
  ...
  terrain-config.js     地形生成常量与可调参数
  terrain-metrics.js    地形诊断指标与导出辅助数据
  ...
  zip-util.js           无压缩 ZIP32 / ZIP64 世界包写入器
tuning/
...
- [D3 Geo](https://github.com/d3/d3-geo) 与 [Jason Davies 的地图旋转示例](https://www.jasondavies.com/maps/rotate/) — 平面投影和拖拽旋转交互参考
