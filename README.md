...
- **多投影平面地图视图**，支持等距柱状、墨卡托、自然地球、Equal Earth 等面积、正射半球与方位等面积投影，可拖拽调整中心经纬度并一键重置默认视角
...
每颗生成的星球都会产生一个 **星球代码**（显示在生成按钮下方），其中编码了随机种子、所有影响生成的滑块、平面地图投影与中心经纬度，以及仍适用于当前板块布局的板块编辑；视图模式、相机位置、图层显示和线框等纯可视化选项不会写入代码。未编辑星球为 24 个字符；通过重建应用过的板块编辑会扩展代码，把切换过的板块包含进去。旧版本的 13-22 字符代码仍受支持，缺失的新参数会使用当前默认值。拖动任意会影响生成或地图的参数时，代码框会实时刷新。分享星球的方法：
...
- **中心经度 / 中心纬度** 滑块（仅平面地图模式）：改变投影观察中心，也可直接在平面地图上用鼠标左键拖拽调整。**重置地图视角** 会恢复默认等距柱状、中心经纬度 0°/0° 和默认缩放。导出不受影响，仍输出标准等距柱状 PNG。
